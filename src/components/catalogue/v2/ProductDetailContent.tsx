...
export function ProductDetailContent({
  ...
  const inStock = variant ? variant.isInStock && variant.stock > 0 : false;
  const canAdd = !!variant && inStock && unitPrice != null;
  ...
  const finalizeAddToCart = () => {
    ...
    toast.success("Added to cart", {
      ...
      action: {
        label: "Go to Cart",
        onClick: () => setTimeout(() => openCart(), 100),
      },
    });
  };
        ...
        <div className="px-6 pt-3 text-center">
          ...
          {step === "configure" && !canAdd && (
            <p className="text-xs text-destructive mt-1">
              {variant ? "Out of stock" : "This combination is unavailable"}
            </p>
          )}
        </div>
      ...
      <div className="px-6 py-4 shrink-0">
        {(step === "configure" || step === "details") && (
          <Button
            ...
            disabled={!canAdd}
          >
            {canAdd ? "Add to Cart" : variant ? "Out of Stock" : "Unavailable"}
          </Button>
        )}
        {step === "purchase-type" && (
